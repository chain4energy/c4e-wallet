...
export enum PublicSaleServiceContext {
  ANY,

}

export class PublicSaleErrorHandler {

  private static instance: PublicSaleErrorHandler;
  ...
  public static getInstance(): PublicSaleErrorHandler {
    if (!PublicSaleErrorHandler.instance) {
      PublicSaleErrorHandler.instance = new PublicSaleErrorHandler();
    }
    return PublicSaleErrorHandler.instance;
  }

  handleError(error?: ErrorData<SaleServiceApplicationError>, publicSaleServiceContext: PublicSaleServiceContext = PublicSaleServiceContext.ANY) {
    if (error) {
      if (error.data) {
        switch (publicSaleServiceContext) {
  ...
  handleCommonNotServiceError(error: ErrorData<SaleServiceApplicationError>) {
    ...
    this.toastService.errorToast(ToastsTypeEnum.USER_SERVICE, message, 1);
  }
